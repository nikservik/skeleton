...
const routes = [
  ...
  { name: 'oferta', path: '/oferta', component: () => import('@/views/Oferta'), meta: { auth: undefined, noVerify: true } },
  ...
  { name: 'verify-link', path: '/verify/:user/:hash', component: () => import('@/views/auth/Verify'), meta: { auth: undefined, noVerify: true }, },
  { name: 'verify', path: '/verify', component: () => import('@/views/auth/Verify'), meta: { auth: true, noVerify: true }, },

  { name: 'profile', path: '/profile', component: () => import('@/views/user/Profile'), meta: { auth: true, noVerify: true } },
  { name: 'support', path: '/support', component: () => import('@/views/user/Support'), meta: { auth: true, noVerify: true } },
...
router.beforeEach((to, from, next) => {
  var checker = (to, from, next) => {
    if (store.getters['auth/canSee'](to.meta)) 
      if (store.getters['auth/needVerification'] && ! to.meta.noVerify)
        next('/verify')
      else
        next()
    else
